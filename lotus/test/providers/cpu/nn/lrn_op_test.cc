...
TEST(LRNTest, LRN_1) {
  LotusIR::NodeArg input1_def("X", &s_typeProto_float), output_def("Y", &s_typeProto_float);
  vector<LotusIR::NodeArg*> input_defs{&input1_def};
  vector<LotusIR::NodeArg*> output_defs{&output_def};

  TestModel model("LRN_1", input_defs, output_defs);
  model.Node().AddAttribute("alpha", .001f);
  model.Node().AddAttribute("beta", .75f);
  model.Node().AddAttribute("bias", 2.0f);
  model.Node().AddAttribute("size", int64_t(5));
  SimpleFloatTest<LRN> test(model);
  ...
  float expected_output[] = {0.5538404f, 0.13542697f, 0.01426445f, 0.40519908f, 0.22705813f,
  ...
  test.AddInput(shape, X);
  test.AddOutput(shape);
  test.Run(shape, expected_output);
}
...
TEST(LRNTest, LRN_2) {
  ...
  TestModel model("LRN_2", input_defs, output_defs);
  model.Node().AddAttribute("alpha", .0001f);
  model.Node().AddAttribute("beta", .75f);
  ...
  float expected_output[] = {0.97537965f, 0.76553655f, 0.44333258f, 0.81260926f,
