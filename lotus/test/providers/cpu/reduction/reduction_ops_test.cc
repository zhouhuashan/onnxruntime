...

{
  ...
  test.AddOutput<float>("reduced", expected_dims, expected_data);
  test.Run();
}

TEST(MathOpTest, ReductionVariationTest) {
  const std::vector<float> &input_data = testcases.input_data_;
  const std::vector<int64_t> &input_dims = testcases.input_dims_;
  OpAttributesResultMap &opAttributesResultMap = testcases.map_op_attribute_expected_;
  ...
  for (auto a : opAttributesResultMap) {
    ...
    TestReduceOp(a.first, input_dims, input_data, attributes.axes_, attributes.keep_dims_,
...
TEST(MathOpTest, ReduceLogSum) {
  ...
  test.AddInput<float>("data", {3, 2, 2}, {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f});
  test.AddOutput<float>("reduced", {3, 1, 2}, {1.09861231f, 2.07944155f, 3.55534792f, 3.87120104f, 4.59511995f, 4.7874918f});
  test.Run();
}
