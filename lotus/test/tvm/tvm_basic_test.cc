...
static void RunSession(InferenceSession& session_object,
  ...
  // prepare inputs
  MLValue ml_value;
  CreateMLValue<double>(TestCPUExecutionProvider()->GetAllocator(MemType::kMemTypeDefault), dims_x, values_x, &ml_value);
  NameMLValMap feeds;
  ...
  const std::vector<double> found(rtensor.Data<double>(), rtensor.Data<double>() + expected_shape.Size());
