...
Status InstanceNorm<T>::ComputeInternal(OpKernelContext* p_op_kernel_context) const {
  ...
  LOTUS_RETURN_IF_ERROR(InstanceNormHelper::ValidateInputs(X, scale, bias));
  ...
  if (N == 1) {
    ...
    LOTUS_RETURN_IF_ERROR(stats_desc.Set(data_desc, CUDNN_BATCHNORM_SPATIAL));
    ...
    CudnnTensor data_desc;
    LOTUS_RETURN_IF_ERROR(data_desc.Set({1, stats_count, image_size, 1}, CudnnTensor::GetDataType<CudaT>()));

    CudnnTensor stats_desc;
    LOTUS_RETURN_IF_ERROR(stats_desc.Set({1, stats_count, 1, 1}, CudnnTensor::GetDataType<CudaT>()));
