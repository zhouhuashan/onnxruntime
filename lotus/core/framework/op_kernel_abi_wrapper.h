...
class OpKernelInfoWrapper : public IMLOpKernelInfo {
 public:
  ...
  ML_API_IMP_(const void*, GetExecutionHandle)
...
class TensorWrapper : public IMLOpTensor {
 public:
  ...
  ML_API_IMP_(MLTensorDataType, GetTensorDataType)
  () const noexcept override;

  ML_API_IMP_(MLBool, IsCPUData)
  () const noexcept override;

  ML_API_IMP_(MLBool, IsDataHandle)
  () const noexcept override;

  ML_API_IMP_(void*, GetData)
  () noexcept override;
...
class OpKernelContextWrapper : public IMLOpKernelContext {
 public:
  ...
  ML_API_IMP(GetInputEdgeType)
  (uint32_t input_index, MLEdgeType* edge_type) const noexcept override;

  ML_API_IMP(GetOutputEdgeType)
  (uint32_t output_index, MLEdgeType* edge_type) const noexcept override;

  ML_API_IMP(GetInputTensor)
  (uint32_t input_index, const IMLOpTensor** tensor) const noexcept override;

  ML_API_IMP(GetOutputTensor)
  (uint32_t output_index, const int64_t* dimension_sizes, uint32_t dimensions, IMLOpTensor** tensor) noexcept override;

  ML_API_IMP_(uint32_t, GetInputCount)
  () const noexcept override;
  ML_API_IMP_(uint32_t, GetOutputCount)
  () const noexcept override;

  ML_API_IMP(AllocateTemporaryData)
  (uint64_t size, void** data) const override;
  ML_API_IMP(FreeTemporaryData)
