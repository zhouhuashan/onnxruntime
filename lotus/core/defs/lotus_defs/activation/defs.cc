...
REGISTER_OPERATOR_SCHEMA(Relu)
    .Description(
        "Relu takes one input data (Tensor<T>) and produces one output "
...
REGISTER_OPERATOR_SCHEMA(PRelu)
    .Description(
        "PRelu takes input data (Tensor<T>) and slope tensor as input, "
        "and produces one output data (Tensor<T>) where the function "
...
REGISTER_OPERATOR_SCHEMA(Affine)
    .Description(
        "Affine takes one input data (Tensor<T>) and produces one output "
...
REGISTER_OPERATOR_SCHEMA(ScaledTanh)
    .Description(
        "ScaledTanh takes one input data (Tensor<T>) and produces one output "
...
REGISTER_OPERATOR_SCHEMA(ThresholdedRelu)
    .Description(
        "Thresholded Relu takes input data (Tensor<T>) and threshold as input, and "
...
REGISTER_OPERATOR_SCHEMA(LogSoftmax)
    .Description(
        "Log Softmax takes one input data (Tensor<T>) and produces one output "
...
REGISTER_OPERATOR_SCHEMA(Hardmax)
    .Description(
        "Compute the hardmax normalized values for each layer in the batch "
        "of the given input. The input is a 2-D tensor (Tensor<float>) of size "
        "(batch_size x input_feature_dimensions). The output tensor has the same shape "
        "and contains the softmax normalized values of the corresponding input. "
        "\n"
    ...
    .TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                    "Constrain input and output types to float tensors.")
    .Attr("axis",
          "Default to 1; describes the axis of the inputs when coerced to 2D; "
...
REGISTER_OPERATOR_SCHEMA(Softsign)
    .Description(
        "Softsign takes one input data (Tensor<T>) and produces one output "
...
REGISTER_OPERATOR_SCHEMA(Softplus)
    .Description(
        "Softplus takes one input data (Tensor<T>) and produces one output "
...
REGISTER_OPERATOR_SCHEMA(Identity)
    .Description(
        "Identity takes one input data (Tensor<T>) and produces one "
