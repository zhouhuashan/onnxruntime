...
REGISTER_OPERATOR_SCHEMA(Reshape)
    ...
    .TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                    "Constrain input and output types to float tensors.")
    .Attr("shape",
          "Tensor of shape declarations for the output. Must be compatible with "
          ...
          "be copied from the input tensor.",
...
REGISTER_OPERATOR_SCHEMA(Transpose)
    .Description(
        "Transpose the input tensor similar to numpy.transpose. For example, "
...
REGISTER_OPERATOR_SCHEMA(Concat)
    .Description(
        "Concatenate takes as input a list of tensors, all of the same shape"
...
REGISTER_OPERATOR_SCHEMA(DepthToSpace)
    .Description(
        "DepthToSpace for 4-D tensors of type T. "
...
REGISTER_OPERATOR_SCHEMA(SpaceToDepth)
    .Description(
        "SpaceToDepth for 4-D tensors of type T. "
