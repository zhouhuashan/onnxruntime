...
REGISTER_OPERATOR_SCHEMA(MaxRoiPool)
    ...
    .Attr("spatial_scale",
          "Multiplicative spatial scale factor to translate ROI "
...
REGISTER_OPERATOR_SCHEMA(LpPool)
    ...
    .Input("X",
           "X Input data tensor from the previous operator; dimensions depend on "
           ...
           "corresponding permutation of dimensions is used in the latter case.",
           "T")
    .Output("Y",
            "Y Output data tensor from L-p pooling across the input tensor. "
            "Dimensions will vary based on various kernel, stride, and pad sizes.",
            "T")
    .TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                    "Constrain input and output types to float tensors.")
...
REGISTER_OPERATOR_SCHEMA(GlobalLpPool)
    .Description(
        "GlobalLpPool consumes an input tensor X and applies lp-pool across the "
    ...
    .Input("X",
           ...
           "T")
    .Output("Y",
            "Y Output data tensor from L-p pooling across the input tensor. Dimensions will "
...
std::function<void(OperatorSchemaSetter&)> LRNDocGenerator() {
  return [=](OperatorSchemaSetter& schema) {
    ...
    schema.TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(double)"},
                          "Constrain input and output types to float tensors.");
    schema.Attr("size",
                "[default 5]: the number of channels to sum over (for cross "
                "channel LRN) or the side length of the square region to sum over (for within "
                "channel LRN)",
                AttrType::AttributeProto_AttributeType_INT, int64_t(5));
...
REGISTER_OPERATOR_SCHEMA(LpNormalization)
    ...
    .Input("input", "Input tensor of any shape", "T")
    .Output("output", "Output tensor of same shape and type as input X.", "T")
    .TypeConstraint("T", {"tensor(float16)", "tensor(float)", "tensor(float)"},
                    "Constrain input and output "
                    "types to float tensors.")
...
REGISTER_OPERATOR_SCHEMA(Embedding)
    ...
    .Input("input",
           "1-D tensor of integers representing indices in the embedding "
           "dictionary with length [N] and values [0, input_dim -1]",
           "T1")
