...
#--------------------------------------------------------------------------
"""
Check OS requirements for onnxruntime python bindings.
"""
import platform
import linecache
import warnings
...
def check_distro_info():
    ...
    if __my_arch__ != '64bit':
        warnings.warn('Unsupported architecture (%s). onnxruntime supports 64bit architecture, only.' % __my_arch__)
    ...
    if __my_system__ == 'windows':
        ...
        if __my_distro_ver__ != '10':
            warnings.warn('Unsupported Windows version (%s). onnxruntime supports Windows 10 and above, only.' % __my_distro_ver__)
        ...
        # warn the user onnxruntime may not work out of the box
        ...
        if __my_distro__ != 'ubuntu' and __my_distro_ver__ != '16.04':
            warnings.warn('Unsupported Linux distribution (%s-%s). onnxruntime supports Ubuntu 16.04 only.' % (__my_distro__, __my_distro_ver__))
    else:
        warnings.warn('Unsupported platform (%s). onnxruntime supports Linux and Windows platforms, only.' % __my_system__)
