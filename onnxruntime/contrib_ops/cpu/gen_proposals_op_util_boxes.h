...
const float BBOX_XFORM_CLIP_DEFAULT = gsl::narrow_cast<float>(log(1000.0 / 16.0));
const float PI = gsl::narrow_cast<float>(3.14159265358979323846);
...
EArrXXt<typename Derived1::Scalar> bbox_transform_upright(
  ...
  if (boxes.rows() == 0) {
    return EArrXX::Zero(0, deltas.cols());
  }

  ORT_ENFORCE_EQ(boxes.rows(), deltas.rows());
  ORT_ENFORCE_EQ(boxes.cols(), 4);
  ORT_ENFORCE_EQ(deltas.cols(), 4);
  ...
  T offset(correct_transform_coords ? static_cast<T>(1.0) : static_cast<T>(0.0));
...
EArrXXt<typename Derived1::Scalar> bbox_transform_rotated(
  ...
  ORT_ENFORCE_EQ(boxes.rows(), deltas.rows());
  ORT_ENFORCE_EQ(boxes.cols(), 5);
  ORT_ENFORCE_EQ(deltas.cols(), 5);
  ...
  if (angle_bound_on) {
    ...
    ORT_ENFORCE(period > 0 && period % 180 == 0);
    auto angles_l = pred_boxes.col(4);
    for (int i = 0; i < angles_l.size(); ++i) {
      if (angles_l[i] < angle_bound_lo) {
        angles_l[i] += T(period);
      } else if (angles_l[i] > angle_bound_hi) {
        angles_l[i] -= T(period);
      }
    }
  }
...
EArrXXt<typename Derived1::Scalar> bbox_transform(
    ...
    const int angle_bound_lo = -90,
    const int angle_bound_hi = 90) {
  ORT_ENFORCE(boxes.cols() == 4 || boxes.cols() == 5);
  if (boxes.cols() == 4) {
    // Upright boxes
...
EArrXXt<typename Derived::Scalar> clip_boxes_upright(
  ...
  // x1 >= 0 && x1 < width
  ret.col(0) = boxes.col(0).cwiseMin(width_f - 1).cwiseMax(0);
  // y1 >= 0 && y1 < height
  ret.col(1) = boxes.col(1).cwiseMin(height_f - 1).cwiseMax(0);
  // x2 >= 0 && x2 < width
  ret.col(2) = boxes.col(2).cwiseMin(width_f - 1).cwiseMax(0);
  // y2 >= 0 && y2 < height
  ret.col(3) = boxes.col(3).cwiseMin(height_f - 1).cwiseMax(0);
...
std::vector<int> filter_boxes_upright(
  ...
  EArrXb keep = (ws >= static_cast<float>(min_size)) && (hs >= static_cast<float>(min_size)) &&
