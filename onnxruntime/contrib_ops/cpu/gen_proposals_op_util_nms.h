...
std::vector<int> nms_cpu_upright(
    ...
    float thresh,
    int64_t topN = -1) {
  ORT_ENFORCE_EQ(proposals.rows(), scores.rows());
  ORT_ENFORCE_EQ(proposals.cols(), 4);
  ORT_ENFORCE_EQ(scores.cols(), 1);
  ORT_ENFORCE_LE(sorted_indices.size(), static_cast<size_t>(proposals.rows()));
...
double bbox_intersection_rotated(
  ...
  ORT_ENFORCE(box1.size() == 5 && box2.size() == 5);
...
Eigen::ArrayXXf bbox_overlaps_rotated(
  ...
  ORT_ENFORCE(boxes.cols() == 5 && query_boxes.cols() == 5);
...
std::vector<int> nms_cpu_rotated(
    ...
    int topN = -1) {
  ORT_ENFORCE_EQ(proposals.rows(), scores.rows());
  ORT_ENFORCE_EQ(proposals.cols(), 5);
  ORT_ENFORCE_EQ(scores.cols(), 1);
  ORT_ENFORCE_LE(sorted_indices.size(), proposals.rows());
...
std::vector<int> nms_cpu(
    ...
    int64_t topN = -1) {
#if defined(CV_MAJOR_VERSION) && (CV_MAJOR_VERSION >= 3)
  ORT_ENFORCE(proposals.cols() == 4 || proposals.cols() == 5);
  if (proposals.cols() == 4) {
    // Upright boxes
