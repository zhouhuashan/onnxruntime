...
Status DeepCpuAttnLstmOp::ComputeImpl(OpKernelContext& context) const {
  ...
  const gsl::span<const T> attn_layer_weights_span = (has_attention_layer) ?
    attn_layer_weights->DataAsSpan<T>() : gsl::span<const T>();
  const gsl::span<const int> memory_seq_lens_span = (attn_memory_seq_lens != nullptr) ?
    attn_memory_seq_lens->DataAsSpan<int>() : gsl::span<const int>();
...
static
Status ValidateRnnInputsWithExtraInputFromState(
    const Tensor& X,
    const Tensor& W,
    const Tensor& R,
    const Tensor* B,
  ...
  if (X_shape.NumDimensions() != 3)
    return LOTUS_MAKE_STATUS(LOTUS, INVALID_ARGUMENT, "Input X must have 3 dimensions only. Actual:", X_shape);

  if (W_shape.NumDimensions() != 3 ||
      W_shape[0] != num_directions ||
      W_shape[1] != hidden_size * WRB_dim_1_multipler ||
      W_shape[2] != input_size)
    return LOTUS_MAKE_STATUS(LOTUS, INVALID_ARGUMENT, "Input W must have shape {",
  ...
  if (R_shape.NumDimensions() != 3 ||
    ...
    return LOTUS_MAKE_STATUS(LOTUS, INVALID_ARGUMENT, "Input R must have shape {",
                             num_directions, ",", WRB_dim_1_multipler, "*", hidden_size, ",",
  ...
  if (B != nullptr) {
    ...
    if (B_shape.NumDimensions() != 2 ||
      ...
      return LOTUS_MAKE_STATUS(LOTUS, INVALID_ARGUMENT, "Input B must have shape {",
  ...
  if (sequence_lens != nullptr) {
    ...
    if (std::any_of(sequence_len_entries.cbegin(),
      ...
      return LOTUS_MAKE_STATUS(
          LOTUS, INVALID_ARGUMENT,
  ...
  if (initial_h != nullptr) {
    ...
    if (initial_h_shape.NumDimensions() != 3 ||
      ...
      return LOTUS_MAKE_STATUS(LOTUS, FAIL, "Input initial_h must have shape {",
