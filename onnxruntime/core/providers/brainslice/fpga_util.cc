...
Status FPGAUtil::InitFPGA(std::unique_ptr<FPGAUtil>* handle) {
  *handle = std::unique_ptr<FPGAUtil>(new FPGAUtil());
  ...
  if (status != FPGA_STATUS_SUCCESS) {
    ...
    return Status(common::ONNXRUNTIME, common::FAIL, "FPGA device initialize failed after retry " + std::to_string(max_retry) + " times.");
  }

  status = FPGA_CreateHandle(&(*handle)->handle_, 0, 0, NULL, NULL);
  if (status != FPGA_STATUS_SUCCESS) {
    (*handle)->status_ = FPGAUtil::FPGA_Status::InitializeFailed;
    return Status(common::ONNXRUNTIME, common::FAIL, "Get FPGA device handle failed with error code:  " + std::to_string(status));
  } else {
...
void WriteMessage(const std::string& p_filename, const void* p_buffer, size_t p_size) {
  FILE* fp;
  fopen_s(&fp, p_filename.c_str(), "wb");
...
Status FPGAUtil::SendSync(const uint32_t p_dstIP, std::function<int32_t(void*, size_t*)> prepare_request, std::function<int32_t(void*, size_t)> process_response) {
  if (status_ != Initialized)
    return Status(common::ONNXRUNTIME, common::FAIL, "FPGA device is not ready");
  ...
  size_t requestSize;
  prepare_request(requestStream.GetCurrent(), &(requestSize = requestStream.GetRemainingBufferSize()));
  ...
  hdr.dst_ep = 1;
  hdr.dst_ip = 0;
  //Accordingt o Adrian, the value of slot field here doesn't matter.
  hdr.slot = 0;
  hdr.vc = 0;
  hdr.ver = 0;
  hdr.req_id_0 = message_id & 0xffff;
  hdr.req_id_1 = (message_id >> 16) & 0xffff;
  memcpy_s(input_buffer, sizeof(NetworkMessageHeader), &hdr, sizeof(NetworkMessageHeader));
  message_id++;
  ...
  FPGA_RETURN_IF_ERROR(FPGA_SendSlotInputBuffer(handle_, slot, sizeof(NetworkMessageHeader) + message_size));
  ...
  memcpy_s(&recvHdr, sizeof(NetworkMessageHeader), response_buffer, sizeof(NetworkMessageHeader));
  FPGA_RETURN_IF_ERROR(FPGA_GetSlotOutputBuffer(slot, &response_buffer));
  //FPGA_RETURN_IF_ERROR(FPGA_GetOutputBufferPointer(handle_, 0, &response_buffer));
...
//TODO: we might need to build a capacity cache, to avoid query capacity multiple times.
Status FPGAUtil::GetCapacities(const uint32_t p_dstIP, BS_Capabilities* out) {
  size_t result_size;
  size_t* p = &result_size;
  auto result = SendSync(p_dstIP, [out](void* buffer, size_t* size) { return BS_CommonFunctions_GetCapabilities_Request(&out->m_bsParameters, buffer, size); },
                         [out, p](void* buffer, size_t size) { return BS_CommonFunctions_GetCapabilities_Response(&out->m_bsParameters, buffer, size, out, p); });
  return result;
}
