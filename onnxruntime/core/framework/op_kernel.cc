...
Tensor* OpKernelContext::Output(int index, const TensorShape& shape) {
  ...
  // so that it's calling IExecutionFrame to create a tensor in the given position with given shape.
  ...
  Status status = execution_frame_->GetOrCreateNodeOutputMLValue(GetOutputArgIndex(index), &parameters.GetTensorShape(), p_ml_value);
...
Status OpKernelContext::GetOrCreateOutputMLValue(int index, MLValue*& p_value) {
  ...
  ORT_ENFORCE(execution_frame_->GetOrCreateNodeOutputMLValue(output_arg_index, &parameters.GetTensorShape(), p_value).IsOK());
  return Status::OK();
}
