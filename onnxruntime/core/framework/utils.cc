...
static Status CopyMLValue(const FeedsFetchesManager::MLValueCopyInfo& copy_info,
  ...
  if (copy_info.copy_provider == nullptr) {
    ...
    if (!target_mlvalue.IsAllocated()) {
      ORT_RETURN_IF_ERROR(utils::AllocateHelper(*copy_info.allocation_provider,
                                                copy_info.allocation_device_id, source_tensor, target_mlvalue));
    }
...
static common::Status CopyInputsAcrossDevices(const std::vector<MLValue>& orig_feeds,
...
static common::Status CopyInputsAcrossDevices(const SessionState& session_state,
  ...
  if (copy_info && !copy_info->empty()) {
    ORT_ENFORCE(num_feeds == copy_info->size());
    needed_copy = true;
...
static common::Status SetupFetchesForExecute(std::vector<MLValue>& fetches,
...
static common::Status CopyOutputsAcrossDevices(const std::vector<MLValue>& fetches,
...
common::Status ExecuteGraph(const SessionState& session_state,
  ...
  if (device_copy_checks.status == DeviceCopyCheck::NoCopy) {
    ...
    if (device_copy_checks.input_copy_needed == DeviceCopyCheck::Copy) {
      ORT_RETURN_IF_ERROR(CopyInputsAcrossDevices(feeds, device_feeds, feeds_fetches_manager.GetFeedsDeviceCopiers()));
    ...
    if (device_copy_checks.output_copy_needed == DeviceCopyCheck::Copy) {
      ORT_RETURN_IF_ERROR(SetupFetchesForExecute(fetches, device_fetches,
    ...
    if (device_copy_checks.output_copy_needed == DeviceCopyCheck::Copy) {
      ORT_RETURN_IF_ERROR(CopyOutputsAcrossDevices(*p_fetches, fetches,
