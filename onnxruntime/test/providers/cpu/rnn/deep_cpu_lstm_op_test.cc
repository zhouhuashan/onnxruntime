...
TEST(LSTMTest, LotusRT_TestLSTMBackward_gpu) {
  const int seq_len = 2, batch_size = 1;
...
TEST(LSTMTest, LotusRT_TestLSTMBatchReallocation) {
  ...
  // updated from LotusRT test so that it's not the same 2 values repeated 6 times each which potentially hides issues
