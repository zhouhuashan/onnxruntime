 ...
 *        of using custom experimental type outside of Lotus.
 *
...
TEST(OpaqueTypeTests, RunModel) {
  ...
  // prepare inputs
  MLValue ml_values;
  CreateMLValue<int64_t>(TestCPUExecutionProvider()->GetAllocator(ONNXRuntimeMemTypeDefault), val_dims, values, &ml_values);

  std::vector<int64_t> ind_dims = {2};
  std::vector<int64_t> indicies = {1, 4};
  MLValue ml_indicies;
  CreateMLValue<int64_t>(TestCPUExecutionProvider()->GetAllocator(ONNXRuntimeMemTypeDefault), ind_dims, indicies, &ml_indicies);

  std::vector<int64_t> shape_dims = {1};
  std::vector<int64_t> shape = {5};
  MLValue ml_shape;
  CreateMLValue<int64_t>(TestCPUExecutionProvider()->GetAllocator(ONNXRuntimeMemTypeDefault), shape_dims, shape, &ml_shape);
