...
static void run(ONNXEnv* env, SessionOptionsWrapper& sf, const wchar_t* test_folder) {
  ...
  std::vector<EXECUTE_RESULT> res;
  {
    ...
    std::vector<ITestCase*> tests = LoadTests({p1}, {}, default_allocator.Upcast());
...
TEST_MODULE_INITIALIZE(ModuleInitialize) {
  ONNXStatusPtr ost = InitializeONNXRuntime(ONNXRUNTIME_LOGGING_LEVEL_kWARNING, "Default", &env);
  if (ost != nullptr) {
