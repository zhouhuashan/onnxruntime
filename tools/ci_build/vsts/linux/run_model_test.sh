...
for version in '28'; do
  ...
  docker run --rm -e AZURE_BLOB_KEY -v $BUILD_ARTIFACTSTAGINGDIRECTORY:/data/a -v $HOME/.ccache:/root/.ccache -v $TOP_SRC_DIR:/data/lotus $docker_image /data/lotus/tools/ci_build/vsts/linux/run_model_test_inside_docker.sh
done
